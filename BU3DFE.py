...
class BU3DFE(DatabaseProcessingUtility):

    def generateTranslationTab(self):
        for subject in getDirectoriesInPath(self.databasePath):
            self.subjectIdentities[subject] = len(self.subjectIdentities) + 1
    ...
    def __init__(self, path, imageType, filePose=['N']):
        ...
        self.subjectIdentities = {}

    def getFilesFromDirectory(self,path):
        returnFiles = []
        for f in glob.glob(path + '/*F3D.'+self.extensionFile[self.imageType]):
    ...
    def feedTemplatesFromList(self,listOfFaces):
        for l in listOfFaces:
            ...
            euTemp = BUTemplate(l, subjectNumber)
        ...
        for subject in getDirectoriesInPath(self.databasePath):
            ...
            for d in directories:
                for ft in self.filePose:
                    if ft in d:
                        subjectNumber = self.subjectIdentities[subject]
                        euTemp = BUTemplate(os.path.join(self.databasePath,subject,d),subjectNumber)
                        self.templates.append(euTemp)
                        break
