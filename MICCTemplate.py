...
class MICCTemplate(Template):
    ...
    def __loadNosetip(self):
        self.faceMarks = [self.image[self.image[:,2].argmax()]]

    def loadImageData(self):
        self.imageLoaded = True
        if (self.rawRepr[-3:]=='ply'):
            imageFace = PlyData.read(self.rawRepr)
            imageFace = np.array([ list(d) for d in imageFace.elements[0].data])
