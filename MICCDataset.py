...
class MICCDataset(DatabaseProcessingUtility):
    ...
    def feedTemplates(self):
        for scanType in self.getDirecotiresInPath(self.databasePath):
            for subject in self.getDirecotiresInPath(os.path.join(self.databasePath,scanType,'processed')):
