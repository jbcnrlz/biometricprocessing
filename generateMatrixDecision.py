...
def main():
    ...
    for idxGroup, group in enumerate(groupsInput):
        filesGroup = group.split('|')
        scores = np.array(readMatrixFile(filesGroup[0]))
        trueLabel = scores[:,-1].flatten()
        scores = scores[:,:-1]
        labels = np.array(readMatrixFile(filesGroup[1]))
        outpuMatrix = np.zeros(scores.shape)
        for idx, t in enumerate(trueLabel):
            outpuMatrix[idx][labels[idx] == t] = 1
