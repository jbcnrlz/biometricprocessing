...
def initialize_model(num_classes, feature_extract, use_pretrained=True,model='resnet50'):
    if model == 'resnet50':
...
def main():
    ...
    parser.add_argument('--classNumber', type=int, default=466, help='Quantity of classes', required=False)
    ...
    muda, input_size = initialize_model(args.classNumber,True,model=args.model)
    ...
    folds = loadFoldsDatasets(args.pathBase, transforms=dataTransform)[0]
    ...
    foldResults.append([])
    train_loader = torch.utils.data.DataLoader(folds[0], batch_size=args.batch, shuffle=True)
    test_loader = torch.utils.data.DataLoader(folds[1], batch_size=args.batch, shuffle=False)
    ...
    for ep in range(args.epochs):
        ...
        cc.add_scalar(args.tensorBoardName+'/loss', sum(lossAcc) / len(lossAcc), ep)
        ...
        cc.add_scalar(args.tensorBoardName+'/Validation_loss', tLoss, ep)
        ...
        if bestForFold < cResult:
            ibr = 'X'
            fName = '%s_best_rank.pth.tar' % ('Resnet18')
        ...
        if bestLossForFold > lossAvg:
            ibl = 'X'
            fName = '%s_best_loss.pth.tar' % ('Resnet18')
            fName = os.path.join(args.folderSnapshots,fName)
            saveStatePytorch(fName, state_dict, opt_dict, ep + 1)
        ...
        cc.add_scalar(args.tensorBoardName+'/accuracy', cResult, ep)
        ...
        if ep % 10 == 0:
            ...
            cc.add_figure(args.tensorBoardName+'/confMatrix', confMat,ep)
